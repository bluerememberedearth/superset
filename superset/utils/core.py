...
def get_base_axis_columns(columns: list[Column] | None) -> list[Column]:
    """Filter and return only the base-axis columns."""
...
def get_non_base_axis_columns(columns: list[Column] | None) -> list[Column]:
    """Filter and return only the non-base-axis columns."""
...
def get_base_axis_labels(columns: list[Column] | None) -> tuple[str, ...]:
    """Extract labels from the base-axis columns."""
...
def get_x_axis_label(columns: list[Column] | None) -> str | None:
    """Return the first base-axis label, or ``None`` if there are none."""
...
def shortid() -> str:
    """Return a short 12-character identifier derived from a UUID."""
...
def to_int(v: Any, value_if_invalid: int = 0) -> int:
    """Safely convert a value to int, returning *value_if_invalid* on failure."""
    try:
        return int(v)
...
def get_query_source_from_request() -> QuerySource | None:
    """Determine the query source from the current HTTP request referrer."""
