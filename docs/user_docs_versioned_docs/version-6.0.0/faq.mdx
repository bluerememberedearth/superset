...
For running a long query from SQL Lab, by default Superset allows it to run as long as 6 hours before it
...
as a [DBAPI driver](https://www.python.org/dev/peps/pep-0249/) defined. Databases that have limited
...
- date-related SQL functions that allow Superset to fetch different time granularities when running
  time-series queries
...
Yes, a public REST API, and the surface of that API is expanding steadily. You can read more about this API and
